...
/* commands
gcc calculateE10.c -I/c/msys64/gmp -L/c/msys64/gmp/lib -lgmp -lpthread -o calc10
...
void* pointerSwap(mpz_t* a, mpz_t* b) {
    mpz_t* temp = (mpz_t*)malloc(mpz_t_size);

    memcpy(temp, b, mpz_t_size);
    memcpy(b, a, mpz_t_size);
    memcpy(a, temp, mpz_t_size);
}
...
void* factorial() {
    ...
    while (!done_calc) {
        ...
        mpz_swap(tmpQueue[qfront], tmp);
        mpz_swap(bfrQueue[qfront], bfact_res[THREADS - 1]);
...
void* approximate() {
    ...
    while (!done_calc) {
        ...
        mpz_fdiv_q(prec, prec, bfrQueue[qrear]);
        if (mpz_cmp_d(prec, 0) == 0) {
            done_calc = 1;
        }
        mpz_mul(res, prec, tmpQueue[qrear]);
        mpz_add(eApprox, eApprox, res);

        iter += len;
        if (iter > 100000 * times) {
            // printf("Mult size: %dKB\tPrec size: %dKB\tResult size: %dKB\t",
            //     (int)(mpz_sizeinbase(bfrQueue[qrear], 2) / (8 * 1024)), (int)(mpz_sizeinbase(prec, 2) / (8 * 1024)), (int)(mpz_sizeinbase(res, 2) / (8 * 1024)));
...
int main(int argc, char const* argv[]) { //arg 1: thread count
    ...
    mpz_init(tmp);
    for (int i = 0; i < queueSize; i++) mpz_inits(tmpQueue[i], bfrQueue[i], NULL);
    ...
    free(tmpQueue);
    free(bfrQueue);
    mpz_clears(prec, eApprox, tmp, NULL);
